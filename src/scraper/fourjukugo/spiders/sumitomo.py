...
class SumitomoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for y in range(1990,2020):
            for i in range(1,3):
                for j in range(1,6):
                    title = response.xpath('//*[@id="anc-'+str(y)+'"]/div[2]/div['+str(i)+']/table/tbody/tr['+str(j)+']/td[1]/strong/text()').extract()
                    meaning = response.xpath('//*[@id="anc-'+str(y)+'"]/div[2]/div['+str(i)+']/table/tbody/tr['+str(j)+']/td[2]/text()').extract()
                    titles.append(title)
